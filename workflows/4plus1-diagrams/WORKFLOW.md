...
Produce a complete Kruchten 4+1 view model where **every view is delivered in canonical format and one or both visual output tracks**:
1. **Canonical diagram-as-code**: Mermaid/PlantUML (per the `4plus1-models` skill).
2. **Editable visual format**: **draw.io** (interactive `.drawio` files), **Miro** (collaboration prompts), or both.

The canonical format and the selected visual track(s) are kept consistent — same component names, same relationships, same scope.
...
The workflow extends — does **not** replace — the `4plus1-models` skill workflow. Run that skill end-to-end, then choose your visual format(s).

1. **Run skill steps 1–4** (mode → audience → context → concerns) from `skills/4plus1-models/SKILL.md`. No changes.
2. **Confirm visual format.** Take the visual track from intake. If it was not provided, ask once: *"How would you like to deliver the diagrams? (A) draw.io, (B) Miro, or (C) both?"* Treat each selected path as an independent track; do not import style/mechanics from one track into the other.
3. **For each view (logical → process → development → physical → scenarios)**:
   ...
   4. Verify component names are consistent across Mermaid/PlantUML and each selected visual format. For Physical view, also verify every relationship endpoint and label from the `.puml` is represented in the draw.io file and/or Miro prompt.
...
### 4c. Output directory precondition

Before writing files, create the selected output folders. Example:

```bash
mkdir -p docs/architecture/diagrams/mermaid docs/architecture/diagrams/drawio docs/architecture/diagrams/miro
```

If only one visual track is selected, create only that track's folder plus `diagrams/mermaid/`.
...
Per-track physical-view checklist details are in `references/notation-drawio.md` and `references/notation-miro.md`.

## 7. Troubleshooting Validation Failures

Use this recovery loop whenever any validator fails.

1. Stop the run and present the validator output exactly as reported.
2. Classify the failure:
   - Cross-view mismatch (`validate-views.py`): canonical names or relationships diverged between views.
   - draw.io validation failure: malformed XML, missing provenance, or shape contract violations.
   - Miro prompt validation failure: markdown syntax issues, missing canonical source section, or incorrect view naming.
3. Regenerate only the affected artefact(s) for the failing view.
4. Re-run the failing validator first.
5. Re-run `validate-views.py` for the full output directory.
...
| 2026-05-16 | Clarified output-selection wording to support one or both visual tracks, added validation troubleshooting flow, and documented maintenance re-sync tooling references. |
