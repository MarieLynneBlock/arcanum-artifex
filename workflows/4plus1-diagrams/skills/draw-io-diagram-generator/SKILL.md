...
| Diagram Type | Template | Description |
|---|---|---|
...
> **For 4+1 views**: use the per-view `.drawio` skeletons from `../../templates/drawio/` (workflow-level) as your starting point instead of the generic templates above. Conventions for each view are in `../../references/notation-drawio.md`.
...
- **Use a template** when the diagram type matches one in `assets/templates/` — copy the template and replace placeholder values. For 4+1 views, use the matching skeleton from `../../templates/drawio/` instead (see routing table in `../../references/notation-drawio.md`).
...
All companion files are in `skills/draw-io-diagram-generator/` (relative to the workflow root):
...
**4+1-specific resources (workflow-level, not inside this skill folder):**

| File | Contents |
|---|---|
| `../../templates/drawio/` | Eight per-view `.drawio` skeletons (logical, process×2, development, physical×3, scenarios) |
| `../../references/notation-drawio.md` | Per-view draw.io conventions: skeleton routing, palette, shape libraries, layout discipline |
