...
> **⛔ Copy the template content below VERBATIM (excluding the outer code fence). Replace `[FILL]` placeholders. ALL 10 attribute rows are MANDATORY per finding. Organise by TIER, not by severity.**
...
- Organised by TIER — no `## Critical Findings` or `## Mitigated` sections
