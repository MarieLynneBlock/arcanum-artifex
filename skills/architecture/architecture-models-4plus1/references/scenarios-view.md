...
For the scenarios view, the Miro prompt **Steps** must specify:
- One frame per scenario with the scenario name as title
- Inside each frame: actors on the left as circle icons, components on the right as rounded rectangles, arrows showing interaction sequence numbered 1–N
- Colour code arrows by interaction type (blue = sync, purple = async, orange = data-write, green = human action)
- One overview frame at the top showing all scenarios as ovals connected to the actors they involve
- One coverage-matrix frame at the bottom as a grid of sticky notes

**Narrowing** should exclude: code structure, deployment topology detail. These are referenced by scenarios but not drawn in this view.

See `examples/synth-claim/miro-prompts/scenarios-view-prompt.md` for a fully-worked example.
