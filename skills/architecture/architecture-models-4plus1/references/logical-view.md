...
Use `templates/miro-prompt-template.md`. For the logical view, specify in the **Steps**:
- Exact number of component boxes
- Exact number of external-system boxes (typically at the top or sides, not centred)
- Exact swimlane / frame structure if you're grouping by domain
- Colour semantics (e.g. blue = our system, grey = external, pink = user-facing)
- Connection types (solid = sync, dashed = async)

See `examples/synth-claim/miro-prompts/logical-view-prompt.md` for a fully-worked example.
