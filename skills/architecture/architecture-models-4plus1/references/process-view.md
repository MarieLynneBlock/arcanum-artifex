...
For the process view, the Miro prompt **Steps** must specify:
- Swimlane count, swimlane order top-to-bottom, and each swimlane's label
- Exact activity-box count per swimlane
- Gateway (diamond) count and placement
- Event (circle) types: start / intermediate / end, and colour coding
- Arrow types: solid for sequence, dashed for message flow between lanes
- Colour coding per swimlane (pick pastel distinguishable colours)

**Narrowing** should exclude: deployment details, code-structure details, the other views' content.

See `examples/synth-claim/miro-prompts/process-view-prompt.md` for a fully-worked example.
