...
For the physical view, the Miro prompt **Steps** must specify:
- Top-level zones (on-prem, cloud, DMZ) as large frames
- VPC / subnet boundaries as nested frames within the cloud zone
- Exact count and placement of compute units per subnet
- Data store icons in their subnets
- Network path arrows (solid for primary path, dotted for management / backup path)
- Security boundaries as coloured outlines (red = public, amber = private, green = isolated)
- Cloud-provider icons from Miro's built-in AWS / Azure / GCP icon set

**Narrowing** should exclude: component responsibilities (covered in logical), runtime call sequences (covered in process), code repos (covered in development).

See `examples/synth-claim/miro-prompts/physical-view-prompt.md` for a fully-worked example.
