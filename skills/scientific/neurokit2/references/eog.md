...
Electrooculography (EOG) measures eye movements and blinks by detecting electrical potential differences generated by eye position changes. EOG is used in sleep studies, attention research, reading analysis, and artefact correction for EEG.
...
**Methods:**
- `'neurokit'`: NeuroKit2 optimised approach (default)
...
**Methods:**
- `'neurokit'`: Butterworth filtering optimised for EOG
...
- **Amplitude velocity ratio (AVR)**: Peak velocity / amplitude
  - Discriminates blinks from artefacts
...
Analyse blink patterns relative to specific events.
...
Analyse blink patterns over extended periods.
...
Visualise processed EOG signal and detected blinks.
...
- ICA-based EOG artefact removal common in EEG preprocessing
...
- Movement: cable artefacts, head motion
...
**EEG artefact correction workflow:**
```python
...
**EEG artefact correction:**
- Blinks contaminate frontal EEG channels
- ICA or regression methods remove EOG artefacts
...
- Kong, X., & Wilson, G. F. (1998). A new EOG-based eyeblink detection algorithm. Behaviour Research Methods, Instruments, & Computers, 30(4), 713-719.
